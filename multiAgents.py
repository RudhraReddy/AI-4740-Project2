...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        # Pacman to all food distance
        for food in foodList:
            disToFood.append(manhattanDistance(food, newPos))

        # Pacman to ghost distance
        for ghost in ghostPos:
            disToGhosts.append(manhattanDistance(ghost, newPos))

        # didnt move return -inf
        if currentGameState.getPacmanPosition() == newPos:
            return -float("inf")

        # ghost is close return -inf
        for distance in disToGhosts:
            if distance < 2:
                return -float("inf")
        ...
        score = 1000 / sum(disToFood) + 10000 / len(disToFood)
        return score
